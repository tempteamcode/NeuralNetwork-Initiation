...
class LeNet(torch.nn.Module):
	def __init__(self):
		...
		self.conv3 = torch.nn.Conv2d(64, 128, 5, 1)
		self.fc1 = torch.nn.Linear(9*9*128, 81)
		self.fc2 = torch.nn.Linear(81, 9)
...
for epoch in range(100):
	...
	for batch_idx, (data, labels, _) in enumerate(train_loader):
		...
		if (batch_idx % STATS_INTERVAL) == 0:
			...
			if ((batch_idx == 0) or STATS_INTERVAL_VALID):
				valid_loss, valid_err, valid_err_all = calcError(model, valid_loader)
			print ('Epoch: %d batch: %5d' % (epoch + 1, batch_idx + 1))
			print (' train-loss: %.3f train-err: %.3f %3.f' % (running_loss / STATS_INTERVAL, train_err, train_err_all))
			if ((batch_idx == 0) or STATS_INTERVAL_VALID):
